...
/**
 * @author adorabled4
 * @className BF : 从所有空闲分区中找到大小最接近请求大小的分区，将其分配给程序使用。
 * @date : 2023-5-15 12:27:40
 **/
public class BF extends Thread implements Runnable {
    ...
    public synchronized long allocate(Process process) {
        ...
        if (partitions.size() == 0 ||
                (partitions.get(0).keySet().iterator().next() < process.getFrameSize())) {
            // 当前没有空闲分区 ||  当前的分区都不能满足
        ...
        for (int i = 0; i < partitions.size(); i++) {
            if (partitions.get(i).keySet().iterator().next() < process.getFrameSize()) {
                // i - 1 块可以满足
                int startFrame = partitions.get(i - 1).values().iterator().next()[0];
                AlgorithmUtil.allocateMem(startFrame, mem, process, startTime);
                return process.getTimeConsume();
            }
        }
