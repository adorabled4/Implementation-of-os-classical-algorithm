...
public class FCFS {
    ...
    public void addProcess(Process p){
        System.out.println(new SimpleDateFormat("MM-dd hh:mm:ss.sss").format(new Date())+ "[arrive]进程到达, 进程ID :"+ p.getPID());
        ...
        while(running==null && (readyQueue.size()>0 || blockQueue.size()>0)){
            ...
            try{
                System.out.println(new SimpleDateFormat("MM-dd hh:mm:ss.sss").format(new Date())+
                        "[running]执行进程 进程ID :"+ running.getPID()+
                        "\t预计耗时(s): "+running.getRunTime());
